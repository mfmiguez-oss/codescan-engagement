...
| R7 | A model steers the context expansion into reading files outside the checkout | requested paths resolved against the checkout and refused if they escape; the suffix fallback that answers a bare `views.py` walks only inside the checkout, compares as a literal string rather than a compiled pattern, and re-applies the same containment check to every match it returns; the symbol search that ranks those matches is bounded to 12 needles and compares each as a literal too, so widening the extractor to bare `login_view` names widens what is searched for and never where | mitigated | test_a_path_outside_the_checkout_is_refused_and_reported, test_resolution_refuses_a_symlink_out_of_the_checkout, test_what_is_not_a_path_resolves_to_nothing, test_a_caller_search_is_literal_never_a_compiled_pattern, test_a_caller_search_refuses_a_symlink_out_of_the_checkout |
...
| R93 | A hostile checkout gets text of its choosing echoed back in the driver's own voice, by planting it where the recorder quotes a rejected evidence snippet into the correction block | the complaint is flattened to a single indented line, length-bounded, and introduced to the model as diagnostic text to read rather than obey | mitigated | test_the_quoted_complaint_cannot_escape_its_block, test_the_quoted_complaint_is_bounded |
