...
| `openhack emit-sarif <target> <run-id> [--out <path>]` | Export triage-accepted findings as SARIF 2.1.0. |
...
`emit-sarif` writes SARIF 2.1.0 for downstream tooling — code-scanning
dashboards, ticketing pipelines, or a triage system. It reads the recorded
triage decisions, not `findings/*.md`, so the export is built from validated
structured data rather than re-parsed prose:

```bash
openhack emit-sarif demo demo-001                    # runs/demo/demo-001/findings.sarif
openhack emit-sarif demo demo-001 --out report.sarif
```

**Only triage-accepted and downgraded findings are exported.** Candidates,
rejected findings, and Semgrep recon hints never appear — the run's own
admission rule is the only gate that decides what counts as a finding, and the
exporter does not add a second one.

What each result carries:

- **`ruleId`** — the CWE the finding names, when it names one; otherwise the
  owning expert id. The expert id is the stable fallback because it comes from
  a fixed registry and does not drift when a model rewords a vulnerability
  class between runs, which would otherwise split one issue into two on rescan.
- **Rule metadata** from the expert's manifest: title, `standard_refs`, and
  tags including `external/cwe/cwe-89` form.
- **Locations** from recorded evidence, which was already validated
  line-by-line against the checkout. A snippet is only ever attached to the
  line it was recorded against; other cited lines become `relatedLocations`
  labelled `source` / `sink`.
- **`versionControlProvenance`** with the repository URL and pinned commit.

> **Severity is lossy in `level`, exact in properties.** SARIF defines only four
> levels, so critical and high both map to `error`. The exact value survives as
> `properties.openhack_severity`, and `properties.security-severity` carries a
> numeric score for consumers that rank by it.

Feeding a triage pipeline (e.g. `triagekit`) that ingests SARIF from a
directory: emit with a `.sarif.json` extension if the consumer globs for
`*.json`.

```bash
openhack emit-sarif demo demo-001 --out /tmp/intake/openhack.sarif.json
```
...
runs/<target>/<run-id>/
  ...
  findings.sarif        SARIF 2.1.0 export (written by emit-sarif).
